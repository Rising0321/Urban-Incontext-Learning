...
from model.MDTall import MaskFlowDiffusionGPT, GPTConfig
...
targets = ["carbon", "house", "crash", "taxi_365", "poi", "src", "dst"]
...
def create_datasets(args):
    ...
    for target in targets:
        data = pd.read_csv(f"{DATA_PATH}/{args.dataset}/{target}.csv")
    ...
    val_dataset = DatasetMAE(test_datas, args.seed, args.few_shot, test=1)
...
def train_one_epoch(model, optimizer, data_loader, epoch, num_epochs, device, global_step, scheduler, ema):
    ...
    for i, (labels_, mask) in enumerate(data_loader):
        ...
        total_loss = masked_noise_loss + args.mask_loss_weight * mask_loss + args.align_loss_weight * cosine_loss
...
def main(args):
    ...
    if args.use_embedding == 0:
        ...
        # embedding = embedding.unsqueeze(0).expand(args.batch_size, -1, -1)

    model = MaskFlowDiffusionGPT(config, region_emb=embedding).to(device)
    ema = deepcopy(model).to(device)  # 创建一个完全相同的EMA模型
    ema.eval()
    ...
    scheduler = get_cosine_schedule_with_warmup(
        ...
        num_training_steps=args.epoch_num * len(train_loader) 
    )

    save_dir = f"{args.checkpoint_dir}/{args.name}-{args.model}-{args.dataset}-{args.seed}"
    ...
    best_val_mae = [123, 123, 123]
    best_test_results = [123, 123, 123]
    for epoch in range(args.epoch_num):
        print(epoch)
        # 训练一个epoch
        global_step = train_one_epoch(model, optimizer, train_loader, epoch, args.epoch_num, device, global_step,
                                      scheduler, ema=ema)
        ...
        if epoch % 20 == 0:
            _, output_json_eval = test(model, test_loader, device, args, num_inference_steps=args.sample_step,
                                       stage="eval",
                                       NUM_GEN_ITERS=1)

            _, output_json_test = test(model, test_loader, device, args, num_inference_steps=args.sample_step,
                                       stage="test",
                                       NUM_GEN_ITERS=1)

            for i in range(len(tasks)):
                task = tasks[i]
                result = output_json_eval[task]
                if result['rmse'] < best_val_mae[i]:
                    best_val_mae[i] = result['rmse']
                    print(f"Best MAE for {task} updated: {best_val_mae[i]:.4f}")
                    best_test_results[i] = output_json_test[task]
                    torch.save(model.state_dict(), f"./{args.dataset}_{task}_best_model.pth")

    print(args.source, best_test_results)
    ...
    # task = "crash"
    # NUM_RUN_ITERS = 500
    # save_path = f"./{args.dataset}_{task}_best_model.pth"
    # best_checkpoint = torch.load(save_path)
    # model.load_state_dict(best_checkpoint)

    # pos_embed = model.pos_embed.squeeze().detach().cpu().numpy()  # 转换为numpy数组
    # pos_embed_save_path = f"./{args.dataset}.npy"
    # np.save(pos_embed_save_path, pos_embed)
    
    # print(task, NUM_RUN_ITERS)
    test_results = test(model, test_loader, device, args, num_inference_steps=args.sample_step, stage="test",
                        NUM_GEN_ITERS=1, NUM_RUN_ITERS=10)
